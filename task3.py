def appearance(intervals):
    ...
    Интервалы – это всегда список из четного количества элементов.
...
"""Могу предположить неточность в тестах, т. к. логично предположить, что таймстемпы должны быть отсортированы.
Если мы соглашаемся с приведённым выше утверждением, то во втором тесте верным результатом работы функции будет 397,
т. к. во входных данных таймстемпы не только не отсортированы, но и пересекаются.
"""
tests = [
